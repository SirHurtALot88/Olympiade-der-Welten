 ...
 *
 * Geprueft werden alle vier Bildschirme, die unter dem Flag Sponsorgeld zeigen:
...
import { getSponsorV2Terms, resetSponsorV2KCache } from "@/lib/sponsor/sponsor-v2-offer-service";
...
/** Ein Spielstand mit einem unterschriebenen V2-Vertrag fuer `teamId`. */
function signedV2State(): { gameState: GameState; teamId: string; offerId: string } {
...
describe("ANZEIGE == SETTLEMENT unter OLY_SPONSOR_V2", () => {
  ...
  it("Finanzen-Detail: Summe UND Aufschluesselung stimmen mit den Settlement-Zeilen ueberein", { timeout: 180_000 }, () => {
    withFlag("1", () => {
      const { gameState, teamId } = signedV2State();
  ...
  it("REGRESSION: ohne Flag zeigt und bucht alles unveraendert die alten Betraege", { timeout: 180_000 }, () => {
    ...
    expect(legacy.hasV2, "ohne Flag darf kein Vertrag V2-Konditionen tragen").toBe(false);
