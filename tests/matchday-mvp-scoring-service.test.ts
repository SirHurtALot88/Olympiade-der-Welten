...
describe("matchday MVP scoring service", () => {
  it("builds 32-team D1 and D2 scoreboards with auto lineups in dry-run", async () => {
    ...
    expect(result.resolveSources.captainSourceStatus).toBe("mapped");
    ...
    expect([...result.d1Scoreboard, ...result.d2Scoreboard].some((row) => (row.intensityModifier ?? 0) !== 0)).toBe(true);
    expect(result.d1Scoreboard.every((row) => row.mutator1Label && !row.mutator1Label.includes("MVP Force"))).toBe(true);
    expect(result.d2Scoreboard.every((row) => row.mutator1Label && !row.mutator1Label.includes("MVP Force"))).toBe(true);
    expect(result.d1TopPlayers).toHaveLength(10);
    expect(result.d2TopPlayers).toHaveLength(10);
    expect([...result.d1TopPlayers, ...result.d2TopPlayers].every((entry) => entry.finalPointsAwarded === Number(((entry.pointsAwarded ?? 0) + (entry.mutatorPpsBonus ?? 0)).toFixed(2)))).toBe(true);
    for (const topPlayers of [result.d1TopPlayers, result.d2TopPlayers]) {
      expect(
        topPlayers.every((entry, index, list) => {
          const previous = list[index - 1];
          return !previous || (previous.finalPointsAwarded ?? Number.NEGATIVE_INFINITY) >= (entry.finalPointsAwarded ?? Number.NEGATIVE_INFINITY);
        }),
      ).toBe(true);
    }
    expect(result.d1TopPlayers.every((entry) => entry.mutatorSelectedTraitLabels?.length === 2)).toBe(true);
    expect(result.d2TopPlayers.every((entry) => entry.mutatorSelectedTraitLabels?.length === 2)).toBe(true);
    expect(result.ppWinners.some((entry) => (entry.mutatorPpsBonus ?? 0) >= 0.3)).toBe(true);
    expect(
      result.ppWinners
        .filter((entry) => (entry.mutatorPpsBonus ?? 0) > 0)
        .every((entry) => (entry.mutatorHitTraitLabels?.length ?? 0) > 0),
    ).toBe(true);
