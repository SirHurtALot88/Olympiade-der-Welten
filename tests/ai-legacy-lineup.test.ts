...
describe("legacy ai lineup suggestion", () => {
  ...
  // ZURUECK AUF null/null — und diesmal ist das die Absicht, nicht ein veralteter Test.
  //
  // Der KI-Pfad setzte ueber applyMutatorTraitsToLineupModifiers / selectBestMutatorTraitsForEntries
  // die zwei Traits mit der staerksten KADER-Abdeckung. Die Wertung gab dieser gespeicherten Auswahl
  // Vorrang vor dem Spieltags-Wurf — jedes KI-Team spielte also mit Mutatoren, die auf seinen eigenen
  // Kader passten (garantierte Treffer), waehrend das menschliche Team kein Auswahlfeld dafuer hat und
  // den blinden Wurf bekam. Mutatoren sind eine Eigenschaft der DISZIPLIN, fuer alle 32 Teams gleich.
  //
  // Der frueher hier zitierte Audit-Hinweis "ai_mutators_unused" ist mit entfernt: er wuerde jetzt bei
  // jedem KI-Team dauerhaft feuern, weil planmaessig kein Team mehr eigene Traits setzt.
  // Siehe tests/mutator-fairness.test.ts.
  it("setzt KEINE kadergenauen Mutator-Traits mehr — der Spieltags-Wurf gilt fuer alle", () => {
    const context = createContext();
    ...
    // Frueher gewann hier "Cool" (deckt p1+p2) und "Ambitious" (deckt den noch unabgedeckten p3) —
    // also genau die Traits, die dieser Kader besitzt. Jetzt bleibt beides leer, und gewertet wird
    // der Wurf des Spieltags.
