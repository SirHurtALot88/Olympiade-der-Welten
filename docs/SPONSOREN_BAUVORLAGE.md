...
| 6 | **TEILWEISE ERLEDIGT (11.08.).** Gebaut ist die **Rangmarke** (`sponsor-rangmarke.ts`): Blöcke aus derselben Leiter, die die Karte ohnehin zeigt; hart = eigener Startblock (große Karten), mild = ein Block darunter; nie darüber. Sie schaltet nach jedem gewerteten Spieltag in `standings-apply-service` — dort entsteht der Tabellenplatz, jede andere Stelle wäre einen Spieltag zu früh oder zu spät. Geld ist nie betroffen. Dabei gefunden und behoben: `blockFuerRang` las die Blockliste in der falschen Richtung, der Meister hätte „Top 28" bekommen. **OFFEN:** die zwei Bonus-Ziele (Frische binär, Achsen-Rang-Bänder) — sie ERSETZEN das V4-Achsensystem und gehören deshalb in denselben Schritt wie dessen Abriss, also in Schritt 8. Zweimal gebaut wäre einmal zu viel. | `sponsor-rangmarke.ts` (neu), `olyDataTypes`, `sponsor-offer-service.ts`, `standings-apply-service.ts` | `sponsor-rangmarke.test.ts` grün (16 Tests) |
...
| 8 | **TEILWEISE ERLEDIGT (11.08.).** Gebaut ist die **KI-Wahl**: sie bewertete das Gebäude vorher **gar nicht** (alle Terme rechnen in Cash, eine Gebäude-Karte war damit schlicht die schlechtere) — und, dabei aufgefallen, auch die **absolute Höhe der Karte** nicht: alle Terme maßen nur Form und Risiko, was richtig war, solange alle Karten denselben Erwartungswert hatten. **OFFEN:** Kartenanzeige (`SponsorOfferCardNewLook`), Übernahme-UI/API, Analytics-Umwidmung, die zwei Bonus-Ziele und der Abriss von Achse/Vorschuss/Golden. | `sponsor-offer-service.ts` | `sponsor-ki-bewertet-gebaeude.test.ts` grün; **gemessen** bei Kasse −30/30/100: reine Cash-Karte 11/2/0 von 32 |
| 9 | **ERSTER LAUF DA (11.08.), Freigabe-Kriterium NICHT erfüllt.** `scripts/messlauf-sponsoren-gebaeude.ts`. Ergebnis am frischen Spiel: Angebot 160 Karten (32 reine Cash, Gebäude 32/64/32 klein/mittel/groß), Verzicht-Spanne **0,8 .. 25,4 C** (Median 6,0). KI-Wahl: 0 von 32 reine Cash-Karte, Gebäude 4/22/6. **Σ Sponsorgeld 1680,3 C gegen Σ Gehälter 2020,0 C = 83 % Deckung** — das Kriterium „Σ Sponsoren deckt Σ Gehälter" ist damit **nicht** erfüllt. Die Gegenrechnung sagt, woran es liegt: der gewählte Cash-Verzicht beträgt 232,3 C, ohne ihn läge die Deckung bei **95 %**. Die Lücke ist also zu etwa zwei Dritteln bewusst gekauft (E5, Rubberband) und zu einem Drittel eine Leiter, die schon vorher knapp unter 100 % lag. **Nicht gemessen:** Bonus-Quoten je Zieltyp (Ziele nicht gebaut) und die Uptime der Rangmarken über die Spieltage — dafür braucht es einen echten Spieltags-Durchlauf. | neues Skript | s. o. |
