...
| MP E2E in CI | 🔴 | `app:smoke-multiplayer-e2e` ist **weiterhin nicht im Tor**. Der alte Blocker (B3, `{d1:0,d2:0}`) ist behoben (`7ea0efa1`) und der Test selbst repariert — beim Nachmessen von Hand faellt er aber an einer NEUEN, tieferliegenden Stelle: `ai-preview lieferte keine entries fuer D-P … "incomplete_roster"`. `D-P` ist eines von Chris' eigenen Teams. Menschliche Teams sind vom Draft ausgeschlossen — **in Solo genauso** (`app/api/new-game/route.ts:83`, „those players draft their own squad"), das ist gewollt und kein Koop-Fehler. Die Luecke liegt im TEST: er ueberspringt die Kaufphase (`buy_players` steht im Raum-Flow vor `lineup`/`arena`) und will direkt in die Arena. Zu beheben im Smoke, nicht im Spiel. Siehe `.github/workflows/ci.yml`. |
