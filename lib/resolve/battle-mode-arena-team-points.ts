...
/**
 * ARENA_RESOLVED_DISCIPLINE_IDS lebt seit A3 (docs/pm-briefings/projektmanager-plan-kampfmodell-
 * umsetzung-20-09.md §3.2) in `lib/battle/arena-resolved-disciplines.ts` — eigene, importfreie
 * Datei, damit `FoundationBattleArenaHost.tsx` ("use client") sie importieren kann, OHNE diese
 * Datei (und damit `arena-headless-runner.ts`s Playwright-/`node:fs`-Importe) in den Client-
 * Bundle zu ziehen. Re-Export hier, damit bestehende Importe von hier unveraendert weiterlaufen.
 * Die volle Herleitung (welche Disziplin warum drin/draussen ist) steht dort, nicht mehr hier.
 */
export { ARENA_RESOLVED_DISCIPLINE_IDS } from "@/lib/battle/arena-resolved-disciplines";
