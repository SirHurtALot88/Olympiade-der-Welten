...
import {
  ...
  isLegacyLineupDraftComplete,
} from "@/lib/lineups/legacy-matchday-readiness";
...
export type MatchdayMvpLineupStatus =
  | "existing_lineup"
  | "auto_lineup_source"
  /** Menschlich gefuehrtes Team — hier stellt niemand ausser dem Spieler auf. */
  | "skipped_manual"
...
export async function runMatchdayMvpScoring(
  ...
  // Wer selbst aufstellt, stellt selbst auf — siehe Begruendung an der Sperre in der Schleife.
  const manualTeamIds = getManualControlTeamIds(nextGameState);
  ...
  for (const team of [...save.gameState.teams].sort((left, right) => left.teamId.localeCompare(right.teamId))) {
    ...
    /**
     * KEIN KI-GRIFF IN SPIELER-TEAMS — der letzte Pfad dieser Sorte.
     *
     * #266 hat zwei Stellen geschlossen, an denen die KI in die Aufstellung eines menschlich
     * gefuehrten Teams geschrieben hat. Diese hier war die dritte: `controlMode` wird oben zwar
     * ermittelt, aber ausschliesslich ins Protokoll geschrieben (`lineupTeams[].controlMode`) —
     * nie als Bedingung. Ein manuelles Team mit fehlender oder unvollstaendiger Aufstellung bekam
     * hier weiterhin eine KI-Aufstellung SAMT KI-gewaehlter Formkarten in den Draft.
     *
     * Das ist nicht bloss Bevormundung: ueber `buildFormCardUsageMap` gilt eine Karte als
     * verbraucht, sobald ihre Id in irgendeinem Draft der Saison steht. Seit #256 fallen
     * verbrauchte Karten aus der Auswahl — der Spieler verliert also Karten, die er nie gespielt
     * hat. Genau so ist es gemeldet worden ("ich habe ploetzlich nicht mehr die moeglichkeit
     * formkarten einzusetzen").
     *
     * Erreichbar ist der Pfad ueber den Cockpit-Knopf (execute + Confirm-Token) und den
     * Debug-Bootstrap — seltener als der Spieltagswechsel aus #266, aber derselbe Schaden.
     */
    if (manualTeamIds.has(team.teamId)) {
      lineupTeams.push({
        teamId: team.teamId,
        teamCode: team.shortCode,
        teamName: team.name,
    ...
    const existingDraftComplete = isLegacyLineupDraftComplete(contextResult.context);
    if (readiness.matchdayReady && contextResult.context.existingDraft && existingDraftComplete) {
      lineupTeams.push({
        teamId: team.teamId,
        teamCode: team.shortCode,
        teamName: team.name,
    ...
    lineupTeams.push({
      ...
      warnings: Array.from(
        new Set([
          "auto_lineup_source",
          ...(replacingIncompleteDraft ? ["incomplete_existing_lineup_replaced"] : []),
          ...preview.warnings,
          ...validation.warnings,
        ]),
      ),
