...
// HSL → sRGB-Komponenten (0..1, gamma-kodiert) — für die color-mix-Nachrechnung.
...
// sRGB-Basis von --nl-panel (#131a26), gamma-kodiert 0..1.
const BASE_PANEL_SRGB: [number, number, number] = [0x13 / 255, 0x1a / 255, 0x26 / 255];

/**
 * Luminanz der TEAM-getönten Panel-Fläche: buildTeamShellThemeVars mischt
 * `color-mix(in srgb, accent 5%, BASE_PANEL)` — bei hellen Marken liegt das
 * ÜBER dem Standard-Panel. GEMESSEN (F1): der Eyebrow-Text stand unter dem
 * S-C-Theme bei 4,44:1, weil accentTextFor gegen das ungetönte Panel
 * rechnete, der Text aber auf der getönten Karte steht. Also wird gegen die
 * echte Fläche gerechnet, nicht gegen die Annahme.
 */
function teamPanelLuminance(accentHsl: string): number {
  const parsed = parseHsl(accentHsl);
  if (!parsed) return PANEL_LUMINANCE;
  const [ar, ag, ab] = hslToSrgb(parsed.h, parsed.s, parsed.l);
  const mix = (acc: number, basis: number) => acc * 0.05 + basis * 0.95;
  const lin = (v: number) => (v <= 0.04045 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4);
  const r = mix(ar, BASE_PANEL_SRGB[0]);
  const g = mix(ag, BASE_PANEL_SRGB[1]);
  const b = mix(ab, BASE_PANEL_SRGB[2]);
  return 0.2126 * lin(r) + 0.7152 * lin(g) + 0.0722 * lin(b);
}
...
export function buildTeamShellThemeVars(code: string | null | undefined): TeamShellThemeVars | null {
  ...
  return {
    ...
    // gemessen gegen die TEAM-getönte Panel-Fläche (siehe teamPanelLuminance).
    "--nl-accent-text": accentTextFor(accent, teamPanelLuminance(accent)),
    "--nl-accent": accent,
    "--nl-accent-2": accent2,
