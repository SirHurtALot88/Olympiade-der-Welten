...
function buildTeamTasks(input: BuildGameInboxInput, visibleTeamIds: Set<string>, createdAt: string) {
  ...
  for (const team of input.gameState.teams) {
    ...
    if (!lineupStatus.hasLineup && controlMode !== "passive") {
      items.push(
        createItem({
    ...
    if (!hasTeamFormCards(input.gameState, team.teamId) && controlMode !== "passive") {
      items.push(
        createItem({
    ...
    if (sellCandidate) {
      items.push(
        createItem({
          ...
          targetView: "market",
          targetParams: { team: team.teamId, player: sellCandidate.entry.playerId, panel: "sell" },
