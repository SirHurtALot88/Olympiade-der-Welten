...
export function buildFinancesViewModel(gameState: GameState, teamId: string | null): FinancesViewModel {
  ...
  const projizierteKomponenten = !sponsorContract
    ...
    : isV3Contract
      ? sponsorSettlementRows
          // Auch NEGATIVE Zeilen gehoeren dazu: eine verfehlte Achse traegt −G/2. Wegzulassen
          // hiesse, dem Team eine Einnahme auszuweisen, die es nie bekommt.
  ...
  const seasonGuv = buildSeasonGuv({
    ...
    apronKonjunkturhebel: apronProjection?.konjunkturhebel ?? null,
    apronGebucht,
    objectiveCashDelta,
