...
export function enforceSeason1SinglePickSpendCapForStep(input: {
  season1OptimumMode: boolean;
  top: SuperstarCapPickCandidate | null;
  lane: AiNeedsPickLane;
  singlePickCap: number | null;
  rankedSelectionCandidates: SuperstarCapPickCandidate[];
  rankedTargetAwareCandidates: SuperstarCapPickCandidate[];
  pickAffordableCash: number;
  stepIndex: number;
}): { top: SuperstarCapPickCandidate | null; warning: string | null; breakLoop: boolean } {
  if (
    !input.season1OptimumMode ||
    input.top?.price == null ||
  ...
  // Narrowed to number by the guard above; capture locally so the closures keep the non-null type.
  ...
  const ordered = [...evaluated]
    ...
    .filter((entry) => {
      ...
      if (input.preserveMinimumPool) {
        // status is already narrowed to non-"eligible" here (handled above at line ~4398)
    ...
    season1OptimumMode && envelopeSlots && envelopeSlots.length > 0
      ? (["cheap_fill", "backup", "depth", "specialist", "core", "star", "superstar"] as AiNeedsPickLane[]).reduce(
          (acc, lane) => {
            ...
            // specialist is a lane but not a distinct cost band — it tracks the core band (see line ~807).
            const costBand: AiNeedsPickCostBand = lane === "specialist" ? "core" : lane;
            if (laneSlots.length === 0) {
              acc.perPickCap[lane] = sharedCostBandCaps[costBand];
              acc.laneSpendTotals[lane] = 0;
              acc.priceCaps[lane] = sharedCostBandCaps[costBand];
              return acc;
            }
...
function buildSimulatedThemeGameState(input: {
  ...
  const syntheticEntries = input.pickedPlayerIds
    ...
    .map((playerId) => {
      ...
      return (
        existing ?? {
          id: `synthetic-${input.teamId}-${playerId}`,
          teamId: input.teamId,
          playerId,
...
function buildTeamEntry(input: {
  ...
  const expectedPrizeSignal =
    input.prizeSignalByTeamId.get(team.teamId) ?? toComparePrizeSignal(null, "missing_source");
  ...
  for (
    ...
    const pickAffordableCash = remainingCash;
      ...
      if (top && normalizeTeamCode(team.shortCode || team.teamId) === "C-S" && isColdSteelThemeBreaker(top.className, top.race)) {
        ...
        const cleanAlternative = rankedSelectionCandidates.find(
          (entry) =>
            ...
            entry.focusTeamStatus !== "blocked" &&
            (entry.price == null || currentTop.price == null || entry.price <= currentTop.price * 1.45 || entry.price <= (currentTop.price ?? 0) + 12) &&
    ...
    if (
      ...
      if (
        ...
        // Corridor is breached and roster-fill/spend-floor pressure forces a pick anyway — but
        // "must spend more" doesn't mean "must overshoot by as much as possible": prefer the
        // cheapest still-quality-gated candidate to minimize how far past the ceiling we land.
        const cheaperOvershootCandidate =
          top != null
            ? rankedQualityCandidates
                .filter(
                  (entry) =>
                ...
                .sort((left, right) => (left.price ?? 0) - (right.price ?? 0))[0] ?? null
            : null;
        if (cheaperOvershootCandidate) {
          top = cheaperOvershootCandidate;
          warnings.push(`Schritt ${stepIndex + 1}: season1_spend_corridor_continue_spend_down_min_overshoot`);
        } else {
    ...
    const superstarCapResult = enforceSeason1SuperstarPickCapForStep({
      season1OptimumMode,
      gameState: input.context.gameState,
      teamId: team.teamId,
      themeSimGameState,
  ...
  if (season1OptimumModeForSteps && (simulatedRosterCount ?? 0) < playerMin) {
    ...
    remainingCash = topUp.remainingCash ?? remainingCash;
  }
