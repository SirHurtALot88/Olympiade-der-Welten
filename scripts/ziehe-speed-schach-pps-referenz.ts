// ===================================================================================
// SPEED-SCHACH-PPS-REFERENZ AUS DEM LIVE-SAVE-ABBILD ZIEHEN
//
// Speed-Schach-Analogon zu scripts/ziehe-gewichtheben-pps-referenz.ts (dort ausfuehrlich
...
// braucht auch fuer Speed-Schach zwei Zahlen JE FELDGROESSE: `iMittel` (Median) und `iKrass`
// (99,5.-Perzentil) des rohen Boxscore-Werts (`MOTOREN["speed-schach"].wert()` = `u.summe`, s.
// WERTUNG_DUELL() in battle-mode.engine.js).
//
// MECHANISMUS: identisch zu Gewichthebens Skript -- `buildArenaTeam()` liefert den echten Kader
// jedes Teams, die besten `n` Spieler nach Speed-Schach-Eignung (`d["speed-schach"]`) werden ueber
// eine synthetische `LineupDraft`-Aufstellung fest in den Speed-Schach-Slot gesetzt,
// `runArenaFixtures()` (mit dem neuen `spieleBuehneDuell()`-Motor-Einstiegspunkt, s.
// arena-headless-runner.ts) simuliert echte Bretter.
//
// KATALOG-STANDARDGROESSE 2, NICHT 6 (nachgesehen, nicht kopiert): `Discipline.playerCount` fuer
// "speed-schach" ist in lib/data/dataAdapter.ts 2 -- BUEHNE_ART["speed-schach"].jeSeite im Motor
// ist trotzdem 6 (die MOTOR-Feldgroesse, die tatsaechlich gespielte "Bretter je Seite"-Zahl). Wie
// bei Hockey (playerCount 5, jeSeite 6 im Feldspiel-Chassis) ist der Katalogwert nur der Fallback
// fuer `ARENA_IMPACT_KONFIG_JE_DISZIPLIN.katalogStandardgroesse`, wenn ein Spieltag ueberhaupt
// keine ermittelbare Feldgroesse hat -- die tatsaechlich GEWUERFELTE Feldgroesse einer Saison
// liegt fuer jede der zwanzig Disziplinen gleichverteilt zwischen 2 und 6, unabhaengig vom
// Katalogwert. Diese Ziehung deckt trotzdem alle fuenf Feldgroessen ab.
//
...
//   OLY_APP_SQLITE_PATH=/tmp/abbild.sqlite npx tsx scripts/ziehe-speed-schach-pps-referenz.ts
//
// Ohne Argumente zieht das Skript ALLE fuenf Feldgroessen (2..6) NACHEINANDER und schreibt direkt
// data/generated/speed-schach-pps-referenz.json. `--feldgroesse=<n>` (Teil-Stand,
// `speed-schach-pps-referenz.partial-<n>.json`) und `--merge` funktionieren wie beim
// Gewichtheben-Skript.
// ===================================================================================
...
const DISZIPLIN = "speed-schach";
const WURZEL = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const ZIEL_DATEI = path.join(WURZEL, "data/generated/speed-schach-pps-referenz.json");
const partialDatei = (n: number) => path.join(WURZEL, `data/generated/speed-schach-pps-referenz.partial-${n}.json`);
...
async function zieheFeldgroesse(gameState: GameState, saveId: string, n: number): Promise<FeldgroessenErgebnis> {
  ...
  if (spielbareTeams.length < 2) {
    throw new Error(`ziehe-speed-schach-pps-referenz: keine zwei Teams mit >= ${n} einsatzfaehigen Spielern gefunden.`);
  }
  ...
  for (let runde = 0; runde < runden; runde += 1) {
    ...
    for (let i = 0; i + 1 < gemischt.length; i += 2) {
      ...
      fixtureInputs.push({
        ...
        seed: `pps-referenz-schach:${n}:${runde}:${heim.teamId}:${gast.teamId}`,
      });
    }
  }
...
function schreibeErgebnis(
  ...
  const ausgabe = {
    disziplin: DISZIPLIN,
    hinweis:
      "iMittel (Median) und iKrass (99,5.-Perzentil) DES ROHEN BRETTPUNKTWERTS " +
      "(MOTOREN['speed-schach'].wert() = u.summe, s. WERTUNG_DUELL() in " +
      "public/mockups/battle-mode.engine.js), JE FELDGROESSE getrennt gezogen -- gebaut von " +
      "scripts/ziehe-speed-schach-pps-referenz.ts gegen echte Liga-Kader (buildArenaTeam()) ueber " +
      "runArenaFixtures()/spieleBuehneDuell(). Gelesen von " +
      "computeIndividualBoxscorePpsFromFixtureResults() in " +
      "lib/resolve/battle-mode-arena-team-points.ts. KATALOG-STANDARDGROESSE IST 2, NICHT 6 " +
      ...
      "rohen Brettpunktwert verschiebt (BUEHNE_ART['speed-schach'].rezept, WERTUNG_DUELL(), " +
    ...
    quelle: { ...quelle, mechanismus: "runArenaFixtures/spieleBuehneDuell gegen echte Liga-Kader (buildArenaTeam)" },
