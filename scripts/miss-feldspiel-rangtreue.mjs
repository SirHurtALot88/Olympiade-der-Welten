// RANGTREUE — die Abnahmezahl fuer die Feldspiel-Motoren (NBA2K-Modell-Runde, 01.09.,
// s. docs/design/battle-mode-nba2k-modell-plan.md).
//
//   node scripts/miss-feldspiel-rangtreue.mjs <disziplin> [spiele] [feldgroessen] [pfad]
//   node scripts/miss-feldspiel-rangtreue.mjs basketball 24 6
//   node scripts/miss-feldspiel-rangtreue.mjs hockey 24 6
//
// Hiess bis zum Hockey-Plan PR 0 miss-basketball-rangtreue.mjs und war auf Basketball
// verdrahtet (docs/design/hockey-rollout-plan.md, Teil H.8). Der alte Name bleibt als
// duenner Vorsatz daneben bestehen und ruft dieselbe Datei mit "basketball" auf — es
// laeuft buchstaeblich derselbe Code, damit die beiden nicht auseinanderlaufen koennen.
//
// WAS BEI EINER VORAB-DISZIPLIN NICHT GEHT. Nur Basketball faehrt heute den Live-Motor.
// Alle anderen Feldspiele rechnen ihr Ergebnis vorab durch: dort gibt es keine
// Manndeckung, keine Wurfdistanzstufen und keinen Deckerabstand im Abwurfmoment. Die
// Rangtreue selbst laesst sich trotzdem bilden (Eignung gegen Impact), die beiden
// Rollenproben V und S nicht. Das Skript SAGT das dann und laesst die Spalten leer,
// statt Nullen zu drucken, die wie Messwerte aussehen.
//
...
// Erster Aufrufwert ist die DISZIPLIN. Der alte Vorsatz miss-basketball-rangtreue.mjs
// schiebt "basketball" davor, damit seine Aufrufe unveraendert weiterlaufen.
const disziplin = String(argumente[0] || "basketball");
const spiele = Number(argumente[1] || 24);
const feldGroessen = String(argumente[2] || "6")
  .split(",")
  .map((x) => Number(x.trim()))
  .filter((x) => Number.isFinite(x) && x > 0);
const seitenPfad = argumente[3]
  ? resolve(argumente[3])
...
function werteAus(daten) {
  ...
  // ABSTANDSBAENDER (Hockey-Plan H.9). Die drei Groessen oben teilen die Wuerfe binaer
  // bei BEDRAENGT_RADIUS = 30 px. Die Wurfformel staffelt den Malus aber STUFENLOS ueber
  // genau diese 30 px, ein Verteidiger auf 29 px kostet also fast nichts. Die binaere
  // Zahl misst die Deckungswirkung deshalb systematisch zu klein (+2,2 Pp gemessen gegen
  // -12,5 Pp laut Kalibrierung der Wurfformel). Hier dieselben Wuerfe nach TATSAECHLICHEM
  // Abstand — die binaeren Spalten bleiben daneben, damit alte Zahlen vergleichbar sind.
  const BAND_LABEL = ["0-10", "10-20", "20-30", ">=30"];
  const bandSumme = BAND_LABEL.map(() => ({ v: 0, t: 0 }));
  for (const s of alleSpiele) {
    for (const p of s.spieler) {
      if (!p.fgBandV) continue;
      for (let b = 0; b < BAND_LABEL.length; b++) {
        bandSumme[b].v += p.fgBandV[b] || 0;
        bandSumme[b].t += (p.fgBandT || [])[b] || 0;
      }
    }
  }
  const baender = bandSumme.map((z, b) => ({
    band: BAND_LABEL[b], n: z.v,
    quote: z.v >= 5 ? rund((z.t / z.v) * 100, 1) : null,
  }));
  ...
  return {
    ...
    probeS: {
      ...
      baender,
      // Spanne zwischen dem engsten und dem weitesten Band — die Zahl, die mit der
      // -12,5 Pp aus der Kalibrierung der Wurfformel vergleichbar ist.
      bandSpanneDPp: (baender[3].quote != null && baender[0].quote != null)
        ? rund(baender[3].quote - baender[0].quote, 1) : null,
    },
    rebound: {
...
await seite.waitForFunction(() => Boolean(window.__arena && window.__arena.feldspielProbe), null, { timeout: 30000 });
...
for (const jeSeite of feldGroessen) {
  ...
  const daten = await seite.evaluate(
    ([d, n, js]) => window.__arena.feldspielProbe(d, { n, jeSeite: js }),
    [disziplin, spiele, jeSeite],
  );
  rohdaten.push(daten);
...
// Kopfzeile traegt die Disziplin gross geschrieben — fuer "basketball" liest sie damit
// weiter "Basketball-Rangtreue" wie vor der Verallgemeinerung.
const titel = disziplin.charAt(0).toUpperCase() + disziplin.slice(1);
console.log(`${titel}-Rangtreue — ${spiele} Spiele je Feldgroesse, Quelle: ${seitenPfad}\n`);
// EHRLICHE FEHLANZEIGE statt stiller Nullen (Hockey-Plan, Auflage fuer PR 0). Bei einer
// Vorab-Disziplin gibt es die halbe Abnahme schlicht nicht — das muss oben stehen, bevor
// jemand die Tabelle darunter fuer eine Messung haelt.
if (rohdaten[0] && rohdaten[0].live === false) {
  console.log(`ACHTUNG: "${disziplin}" faehrt den VORAB-Pfad, nicht den Live-Motor.`);
  console.log("Die Rangtreue (Eignung gegen Impact) gilt; die Rollenproben V und S NICHT.");
  console.log("Es fehlt im Vorab-Pfad:");
  for (const f of rohdaten[0].fehlend) console.log("  - " + f);
  console.log("");
}
...
// H.9: Trefferquote gegen den TATSAECHLICHEN Deckerabstand. Die binaere S-Spalte oben
// verwaessert die Deckungswirkung, weil sie einen Verteidiger auf 29 px wie einen auf
// Tuchfuehlung zaehlt; die Wurfformel staffelt den Malus stufenlos ueber diese 30 px.
for (const e of ergebnisse) {
  const b = e.probeS.baender;
  if (!b || b.every((z) => z.quote == null)) continue;
  console.log(`\njeSeite ${e.jeSeite} — FG% nach Deckerabstand (px): ` +
    b.map((z) => `${z.band} ${z.quote == null ? "—" : z.quote + "%"} (n=${z.n})`).join("  "));
  console.log(`   Spanne weitestes minus engstes Band: ` +
    (e.probeS.bandSpanneDPp == null ? "—"
      : (e.probeS.bandSpanneDPp >= 0 ? "+" : "") + e.probeS.bandSpanneDPp + " Pp") +
    "   (Kalibrierung der Wurfformel: -12,5 Pp offen gegen stark bedraengt)");
}
