// ===================================================================================
// GEWICHTHEBEN-PPS-REFERENZ AUS DEM LIVE-SAVE-ABBILD ZIEHEN
//
// Gewichtheben-Analogon zu scripts/ziehe-basketball-pps-referenz.ts (dort ausfuehrlich
// begruendet -- hier nur die Unterschiede). Auftrag: docs/design/gewichtheben-produktivierung.md
// Schritt 5 ("Individuelle PPs fuer Gewichtheben").
//
// `computeIndividualBoxscorePpsFromFixtureResults()` (lib/resolve/battle-mode-arena-team-points.ts)
// braucht auch fuer Gewichtheben zwei Zahlen JE FELDGROESSE: `iMittel` (Median) und `iKrass`
// (99,5.-Perzentil) des rohen Boxscore-Werts. Bei Gewichtheben ist dieser Rohwert NICHT ein
// abstrakter "Impact"-Score wie bei Basketball, sondern der ECHTE Zweikampf in Kilogramm
// (`MOTOREN.gewichtheben.wert()` liefert `u.summe`, s. battle-mode.engine.js
// `baueHebenDuelle()`) -- die Kurve braucht die beiden Anker trotzdem, weil das absolute
// Kilogramm-Niveau vom Kader (Attributverteilung der Liga) abhaengt, genau wie Basketballs
// Impact-Niveau von der Liga abhaengt.
//
// MECHANISMUS: identisch zu Basketballs Skript -- `buildArenaTeam()` liefert den echten Kader
// jedes Teams, die besten `n` Spieler nach Gewichtheben-Eignung (`d.gewichtheben`) werden ueber
// eine synthetische `LineupDraft`-Aufstellung fest in den Gewichtheben-Slot gesetzt,
// `runArenaFixtures()` (mit dem neuen `spieleBuehneHeben()`-Motor-Einstiegspunkt, s.
// arena-headless-runner.ts) simuliert echte Zweikaempfe.
//
// KLEINERE STICHPROBE ALS BASKETBALL, EXPLIZIT DOKUMENTIERT: Basketballs Referenz zog 300+
// Fixtures JE Feldgroesse (2..6, insgesamt ueber 1500 Fixtures, rund zwei Stunden Laufzeit).
// Gewichthebens Katalog-Feldgroesse ist FEST bei 6 (s. `Discipline.playerCount` in
// lib/data/dataAdapter.ts und `BUEHNE_ART.gewichtheben.jeSeite` im Motor) -- eine andere
// Feldgroesse kann nur durch Unterzahl entstehen (ein Team stellt weniger als 6 einsatzfaehige
// Heber, s. `baueHebenDuelle()`s `n=Math.min(mine.length,gegner.length)`), nicht durch eine
// gewuerfelte Katalog-Groesse wie bei Basketball. Diese Ziehung deckt trotzdem 2..6 ab (fuer den
// Unterzahl-Fall), aber mit EINER KLEINEREN Stichprobe je Feldgroesse (`FIXTURES_ZIEL` unten) --
// eine bewusste Zeitbudget-Entscheidung fuer die Erstziehung, s. docs/design/
// gewichtheben-produktivierung.md ("Was NICHT getan wurde"). p99,5 ist bei kleinerer Stichprobe
// weniger stabil geschaetzt als Basketballs Referenz; eine spaetere, groessere Nachziehung
// (--feldgroesse=<n> + --merge, identisches Muster wie bei Basketball) ist ohne Code-Aenderung
// moeglich, sobald mehr Rechenzeit zur Verfuegung steht.
//
...
//   OLY_APP_SQLITE_PATH=/tmp/abbild.sqlite npx tsx scripts/ziehe-gewichtheben-pps-referenz.ts
//
// Ohne Argumente zieht das Skript ALLE fuenf Feldgroessen (2..6) NACHEINANDER und schreibt direkt
// data/generated/gewichtheben-pps-referenz.json. `--feldgroesse=<n>` (Teil-Stand,
// `gewichtheben-pps-referenz.partial-<n>.json`) und `--merge` funktionieren wie beim
// Basketball-Skript.
// ===================================================================================
...
const DISZIPLIN = "gewichtheben";
const WURZEL = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const ZIEL_DATEI = path.join(WURZEL, "data/generated/gewichtheben-pps-referenz.json");
const partialDatei = (n: number) => path.join(WURZEL, `data/generated/gewichtheben-pps-referenz.partial-${n}.json`);
...
// Feldgroesse, eine bewusste Zeitbudget-Entscheidung fuer die Erstziehung.
...
async function zieheFeldgroesse(gameState: GameState, saveId: string, n: number): Promise<FeldgroessenErgebnis> {
  ...
  if (spielbareTeams.length < 2) {
    throw new Error(`ziehe-gewichtheben-pps-referenz: keine zwei Teams mit >= ${n} einsatzfaehigen Spielern gefunden.`);
  }
  ...
  for (let runde = 0; runde < runden; runde += 1) {
    ...
    for (let i = 0; i + 1 < gemischt.length; i += 2) {
      ...
      fixtureInputs.push({
        ...
        seed: `pps-referenz-heben:${n}:${runde}:${heim.teamId}:${gast.teamId}`,
      });
    }
  }
  ...
  const t0 = Date.now();
  // In Batches, aus demselben Speichergrund wie Basketballs Skript (s. dortiger Kommentar).
...
function schreibeErgebnis(
  ...
  const ausgabe = {
    disziplin: DISZIPLIN,
    hinweis:
      "iMittel (Median) und iKrass (99,5.-Perzentil) DES ECHTEN ZWEIKAMPFS IN KILOGRAMM " +
      "(MOTOREN.gewichtheben.wert() = u.summe, s. baueHebenDuelle() in " +
      "public/mockups/battle-mode.engine.js), JE FELDGROESSE getrennt gezogen -- gebaut von " +
      "scripts/ziehe-gewichtheben-pps-referenz.ts gegen echte Liga-Kader (buildArenaTeam()) ueber " +
      "runArenaFixtures()/spieleBuehneHeben(). Gelesen von " +
      "computeIndividualBoxscorePpsFromFixtureResults() in " +
      "lib/resolve/battle-mode-arena-team-points.ts. KLEINERE STICHPROBE ALS BASKETBALLS " +
      "REFERENZ (60 statt 300+ Fixtures je Feldgroesse, s. Skript-Kopfkommentar) -- eine " +
      "Zeitbudget-Entscheidung der Erstziehung (04.09.2026), keine methodische. Neu ziehen nach " +
      "jeder Aenderung, die den rohen Zweikampf verschiebt (HEBEN_*-Konstanten, " +
      "BUEHNE_ART.gewichtheben.rezept, Kadergenerierung/Attributniveau der Liga).",
    gezogenAm: new Date().toISOString(),
    motorSha1: ermittleMotorSha1(),
    repoCommit: ermittleRepoCommit(),
    quelle: { ...quelle, mechanismus: "runArenaFixtures/spieleBuehneHeben gegen echte Liga-Kader (buildArenaTeam)" },
