...
export default function MatchdayArenaV2Client(props: MatchdayArenaV2ClientProps) {
  ...
  // 7d: injury/availability display. `context.rosterPlayers` (LegacyRosterPlayerRef) is
  // already loaded from playerAvailabilityState (see legacy-lineup-local-service.ts), so no
  // other data source needs to change -- it's just never rendered in this client today.
  // A player's injuryStatus stays "injured" both on the matchday they got hurt (still
  // available that day) and the following blocked matchday (see
  // getPlayerAvailabilityView / fatigue-injury-service.ts); availabilityBlocker is what
  // tells the two apart, so we use it purely as a derived signal (no team/player hardcodes).
  const rosterInjuryGroups = useMemo(() => {
    const roster = context?.rosterPlayers ?? [];
    const newlyInjured = roster.filter((player) => player.injuryStatus === "injured" && player.availabilityBlocker == null);
    const blockedByInjury = roster.filter((player) => player.availabilityBlocker === "player_injured_unavailable");
    const recovering = roster.filter((player) => player.injuryStatus === "recovering");
    return { newlyInjured, blockedByInjury, recovering };
  }, [context?.rosterPlayers]);
  const injuryPerformanceMalusPercent = Math.round((1 - INJURY_PERFORMANCE_MULTIPLIER) * 100);
  const hasRosterInjuryNews =
    rosterInjuryGroups.newlyInjured.length > 0 ||
    rosterInjuryGroups.blockedByInjury.length > 0 ||
    rosterInjuryGroups.recovering.length > 0;
  ...
  return (
    <div className={`arena-v2-shell is-act-${arenaAct}${isArenaEventMode ? " is-event-mode" : ""}${isFocusedBoardMode ? " is-focused-board" : ""}${broadcastFocusMode ? " is-broadcast-mode" : ""}`}>
      ...
      {hasRosterInjuryNews ? (
        <section className="arena-v2-feedback-grid" aria-label="Verletzungen" data-testid="arena-v2-injury-panel">
          {rosterInjuryGroups.newlyInjured.slice(0, 3).map((player) => (
            <article key={`arena-v2-injury-new-${player.id}`} className="arena-v2-feedback is-error">
              <strong>Neue Verletzung</strong>
              <span>
                {player.name} verletzt sich in diesem Spieltag
                {player.injuryUntilMatchday ? ` · pausiert bis ${player.injuryUntilMatchday}` : ""}
                {player.injuryRiskLabel ? ` · ${player.injuryRiskLabel}` : ""}
              </span>
            </article>
          ))}
          {rosterInjuryGroups.blockedByInjury.slice(0, 3).map((player) => (
            <article key={`arena-v2-injury-blocked-${player.id}`} className="arena-v2-feedback is-warning">
              <strong>Verletzt · pausiert</strong>
              <span>
                {player.name} faellt aus{player.injuryUntilMatchday ? ` bis ${player.injuryUntilMatchday}` : ""}
              </span>
            </article>
          ))}
          {rosterInjuryGroups.recovering.slice(0, 3).map((player) => (
            <article key={`arena-v2-injury-recovering-${player.id}`} className="arena-v2-feedback is-warning">
              <strong>Erholt sich</strong>
              <span>{player.name} ist wieder einsatzbereit, Belastung im Blick behalten.</span>
            </article>
          ))}
        </section>
      ) : null}
      ...
      <section className="panel arena-v2-lower-panel">
        ...
        <section className="arena-v2-lower-section arena-v2-insight-panel arena-v2-act-result">
          ...
          <div className="arena-v2-insight-grid">
            ...
            <article className="arena-v2-insight-card" data-testid="arena-v2-injury-malus-card">
              <span>Verletzungs-Malus</span>
              <strong>-{injuryPerformanceMalusPercent}% Score</strong>
              <small>
                {rosterInjuryGroups.newlyInjured.length > 0
                  ? `Gilt fuer ${rosterInjuryGroups.newlyInjured.length} frisch verletzte${rosterInjuryGroups.newlyInjured.length === 1 ? "n" : ""} Spieler diesen Spieltag.`
                  : "Greift automatisch, sobald ein Spieler an diesem Spieltag verletzt wird."}
              </small>
            </article>
          </div>
          {effectiveBoardMode !== "total" ? (
